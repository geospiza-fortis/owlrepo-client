<script>
  ...
  onMount(async () => {
    let resp = await fetch(`__OWLREPO_URL__/api/v1/query/curation_candidate`);
  ...
  async function fetchTask(task_id, screenshot_sha1) {
    ...
    let base = `__OWLREPO_URL__/api/v1/data/${task_id}`;
  ...
  async function submitTask() {
    ...
    let resp = await fetch(`__OWLREPO_URL__/api/v1/submit_curate_task`, {
      headers: {
