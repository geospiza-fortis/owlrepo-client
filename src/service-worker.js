 ...
 */
async function sessionFetchOrCache(request, postMessage) {
  // session cache
  dev &&
    postMessage({
      reason: "log request",
  ...
  const cache = await caches.open(`session${version}`);
    ...
    event.respondWith(
      (async () => {
        ...
        const postMessage = (msg) =>
          dev && client ? client.postMessage(JSON.stringify(msg)) : () => {};
